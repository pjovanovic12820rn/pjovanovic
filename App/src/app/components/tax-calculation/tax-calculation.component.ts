...
export class TaxCalculationComponent implements OnInit {
  ...
  getCurrTaxData(): void {
	...
	filterTransactions(): void {
		this.filteredUserTax = this.userTax.filter(
			(transaction) =>
	...
	calculateTax(): void {
		...
		this.alertService.showAlert("info", "Processing tax calculation...");
		...
		this.taxService.calculateTax().subscribe({
			next: (result) => {
				...
				if (!trimmed) {
					this.alertService.showAlert(
						"warning",
						"Tax calculation finished, but no response was returned.",
					);
				} else if (trimmed.toLowerCase().includes("error")) {
					this.alertService.showAlert("error", trimmed);
				} else {
					this.alertService.showAlert("success", trimmed);
          this.getCurrTaxData()
				}
			},
			error: (err) => {
				...
				this.alertService.showAlert(
					"error",
					err.message || "An error occurred during tax calculation.",
				);
			},
		});
	}
