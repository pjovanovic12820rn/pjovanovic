...
export class SecuritiesComponent implements OnInit {
		...
		this.securityService.getSecurities().subscribe({
			...
			error: (err) => {
				...
				this.alertService.showAlert(
					"error",
					"Failed to refresh securities list.",
				);
				this.isLoading = false;
			},
		});
	...
	protected testMode(): void {
		this.securityService.getSecurities().subscribe({
			next: (res) => {
				this.alertService.showAlert(
					"success",
					"Successfully toggled test market mode",
				);
			},
			error: (err) => {
				...
				this.alertService.showAlert(
					"error",
					"Failed to toggle test market mode",
				);
				this.isLoading = false;
			},
		});
	...
	private filterSecurities(): void {
		...
		if (this.selectedSecurityType !== "All") {
			tempFilteredSecurities = tempFilteredSecurities.filter(
				(security) => security.type === this.selectedSecurityType,
			);
		}
		...
		if (this.exchangePrefix) {
			tempFilteredSecurities = tempFilteredSecurities.filter((security) =>
				security.ticker
					.toLowerCase()
					.startsWith(this.exchangePrefix.toLowerCase()),
			);
		}
		...
		if (this.settlementDateFilter) {
			tempFilteredSecurities = tempFilteredSecurities.filter((security) => {
				if (security.type === "Future") {
					return (
						security.settlementDate?.includes(this.settlementDateFilter) ??
						false
					);
				}
				return true;
